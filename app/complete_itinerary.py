...
def is_theme_park_day(day_plan: StructuredDayPlan) -> bool:
    """Check if this is a theme park day based on landmark names and duration"""
...
async def add_restaurants_to_day(
    ...
    """Add restaurants strategically along the landmark route"""
    
    debug_print(f"🍽️ Adding restaurants to Day {day_plan.day} ({'theme park' if is_theme_park else 'regular'} day)")
    ...
    if is_theme_park:
        ...
        # For theme park days, we still want proper meal timing
        # Breakfast before park, lunch in/near park, dinner after park
        meal_schedule = [
            ("breakfast", "08:00", "45m"),
            ("lunch", "12:30", "1h"),    # Lunch break during park visit
            ("dinner", "18:00", "1.5h")  # Dinner after park closes
        ]
        
        for meal_type, start_time, duration in meal_schedule:
            # Use theme park location for restaurant search
            theme_park_location = landmarks[0].location if landmarks else None
            
            restaurant_data = await search_and_get_restaurant(
                places_client, destination, meal_type, theme_park_location, used_restaurants
            )
            ...
            if restaurant_data:
                ...
                used_restaurants.add(restaurant_data["name"])
                debug_print(f"   🍽️ {meal_type.title()}: {restaurant_data['name']} at {start_time} ({duration})")
    
    else:
        # Regular day: Insert restaurants strategically along the landmark route
        debug_print(f"🗺️ Regular day - inserting restaurants along landmark route")
        
        # Calculate strategic meal times based on landmark schedule
        if landmarks:
            ...
            # Breakfast: 30 minutes before first landmark
            breakfast_time = max(480, first_landmark_start - 30)  # No earlier than 8:00 AM
            
            # Lunch: Find a good break point around midday
            lunch_time = find_optimal_meal_time(landmarks, "lunch", 720, 780)  # 12:00-13:00 target
            
            # Dinner: After last landmark or around 6 PM
            dinner_time = max(last_landmark_end + 30, 1080)  # At least 6:00 PM
            ...
            debug_print(f"📅 Strategic meal times calculated:")
            for meal_type, time_minutes, duration in meal_schedule:
                time_str = f"{time_minutes // 60:02d}:{time_minutes % 60:02d}"
                debug_print(f"   🕐 {meal_type.title()}: {time_str} ({duration})")
            
            # Find restaurants for each meal
            for meal_type, time_minutes, duration in meal_schedule:
                time_str = f"{time_minutes // 60:02d}:{time_minutes % 60:02d}"
                
                # Find the closest landmark for location context
                closest_landmark = find_closest_landmark_to_time(landmarks, time_minutes)
                search_location = closest_landmark.location if closest_landmark else None
                
                restaurant_data = await search_and_get_restaurant(
                    places_client, destination, meal_type, search_location, used_restaurants
                )
                ...
                if restaurant_data:
                    ...
                    restaurants.append(restaurant_block)
                    used_restaurants.add(restaurant_data["name"])
                    debug_print(f"   🍽️ {meal_type.title()}: {restaurant_data['name']} at {time_str} ({duration})")
        
        else:
            # Fallback if no landmarks
            debug_print("⚠️ No landmarks found, using default meal times")
            meal_times = [
                ("breakfast", "08:00", "45m"),
                ("lunch", "12:30", "1h"),
                ("dinner", "18:00", "1.5h")
            ...
            for meal_type, start_time, duration in meal_times:
                restaurant_data = await search_and_get_restaurant(
                    places_client, destination, meal_type, None, used_restaurants
                )
    ...
    debug_print(f"✅ Day {day_plan.day}: Added {len(restaurants)} restaurants")
...
async def complete_itinerary_from_selection(
    ...
    try:
        ...
        used_restaurants = set()
        if not places_client:
            debug_print("⚠️ No places client available - skipping restaurant addition")
        else:
            debug_print("🍽️ Adding restaurants to each day...")
            resto_start_time = time.time()
            for day_plan in itinerary.itinerary:
                is_theme_park = is_theme_park_day(day_plan)
                try:
                    enhanced_day = await add_restaurants_to_day(
                        day_plan,
                        places_client,
                        destination,
                        used_restaurants,
                    ...
                    day_plan.blocks = enhanced_day.blocks
                    debug_print(f"✅ Day {day_plan.day}: {len([b for b in day_plan.blocks if b.type == 'restaurant'])} restaurants added")
                except Exception as e:
                    debug_print(f"❌ Error adding restaurants to Day {day_plan.day}: {e}")
                    logger.error(f"Error adding restaurants to Day {day_plan.day}: {e}")
            resto_end_time = time.time()
            performance_metrics["timings"]["restaurant_addition"] = round(resto_end_time - resto_start_time, 2)
            debug_print(f"✅ Restaurants added in {resto_end_time - resto_start_time:.2f} seconds")

        # Enhance landmarks with Google Places data (cost-efficiently)
        debug_print("🏛️ Enhancing landmarks with Google Places data...")
        enhance_start_time = time.time()
        itinerary, api_calls = await enhance_landmarks_cost_efficiently(itinerary, places_client, destination)
        enhance_end_time = time.time()
        performance_metrics["timings"]["landmark_enhancement"] = round(enhance_end_time - enhance_start_time, 2)
        performance_metrics["costs"]["google_places"]["enhancement_api_calls"] = api_calls
        debug_print(f"✅ Landmark enhancement completed in {enhance_end_time - enhance_start_time:.2f} seconds")
...
def add_theme_park_meals(day_activities: List[Dict], restaurants: List[Dict]) -> List[Dict]:
    """Add strategically timed meals for theme park days"""
    if not restaurants:
        return day_activities
    
    # Theme park meal strategy: breakfast early, lunch mid-day, dinner after park
    meal_times = [
        ("breakfast", "08:00"),
        ("lunch", "12:30"),     # Changed from 12:00 to 12:30
        ("dinner", "18:00")     # Changed from 17:00 to 18:00 to reduce gap
    ]
